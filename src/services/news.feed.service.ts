...
export class NewsFeedService {
  constructor(@InjectRepository(NewsFeedEntity) private feedRepo: Repository<NewsFeedEntity>) {
  }

  async postNewsFeed(body: NewsFeedEntity, req: any): Promise<NewsFeedEntity> {
    body.date = new Date();
    body.user_id = req.user.id;
    return this.feedRepo.save(body).then(res => {
      delete res.user_id;
      return res;
    });
  }
  ...
  async getAllNewsFeed(id: number): Promise<NewsFeedEntity[]> {
    const results = await this.feedRepo.find({ relations: ['user', 'likes'] });
  ...
  async getMyNewsFeed(id: number): Promise<NewsFeedEntity[]> {
    return await this.feedRepo.find({ relations: ['user'], where: { user_id: id } });
  }

  async getNewsFeed(id: number): Promise<NewsFeedEntity> {
    const feed = await this.feedRepo.findOne(id, { relations: ['user'] });
  ...
  async updateNewsFeed(body: FeedUpdateValidation, req: any): Promise<NewsFeedEntity> {
    const feed = await this.feedRepo.findOne(body.feed_id);
    if (feed == undefined) throw MessageConstant.not_found_news_feed;
  ...
  async deleteNewsFeed(id: number, req: any): Promise<NewsFeedEntity> {
    ...
    return await this.feedRepo.remove(feed);
  }

  async likeOrUnlikeNewsFeed(id: number, req: any): Promise<any> {
    const newsFeed = await this.feedRepo.findOne(id, { relations: ['likes'] });
    ...
    return await this.feedRepo.save(newsFeed);
  }
}